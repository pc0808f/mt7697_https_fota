...
static int32_t _fota_https_retrieve_get(char* get_url, char* buf, uint32_t len)
{
...
int8_t fota_download_by_https(char *param, const char server_cert[], int server_cert_len)
{
    ...
    _s_fota_httpclient.server_cert = server_cert;
    _s_fota_httpclient.server_cert_len = server_cert_len;

    LOG_I(fota_dl_api, "[FOTA DL] server_cert_len=%d.\r\n " ,_s_fota_httpclient.server_cert_len);
#endif    
    ret = httpclient_connect(&_s_fota_httpclient, get_url);
    if (!ret) {
        ret = _fota_https_retrieve_get(get_url, buf, FOTA_BUF_SIZE);
    }else {
